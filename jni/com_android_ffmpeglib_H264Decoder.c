	...
	while (avpkt.size > 0) {
		...
		if (got_picture && jout) {
			...
			if (DEBUG) {
				...
				__android_log_print(ANDROID_LOG_ERROR, TAG,
						"H264Decoder get framepicture->linesize[0] %d picture->linesize[1] %d\n",
						picture->linesize[0], picture->linesize[1]);
			}
			DisplayYUV_16((int*) Pixel, picture->data[0], picture->data[1],
					picture->data[2], context->width, context->height,
					picture->linesize[0], picture->linesize[1]);
...
JNIEXPORT jint JNICALL Java_com_android_ffmpeglib_H264Decoder_geth(
		JNIEnv * en, jobject jh264) {
	if(context)
...
JNIEXPORT jint JNICALL Java_com_android_ffmpeglib_H264Decoder_getw(
		JNIEnv * en, jobject jh264) {
	if(context)
